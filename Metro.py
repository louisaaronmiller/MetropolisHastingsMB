    ...
    if C:
        ...
        for i in range(1,N):
            ...
            if i < 10000:
                continue
            if i >= 10000:
                denominator = 1/(np.exp(sample[i-1]) * Boltz(m,T,sample[i-1]))
                run_C += denominator
                M_reciprocal_run_C = i * (1/(run_C + eps))
...
def Metro_expectation2(v,T,m,lamb,N= 1000000, C=True):
    ...
    if C:
        ...
        for i in range(1,N):
            # Calculating C
            denominator = np.sin(sample[i-1])/(sample[i-1] * Boltz(m,T,sample[i-1]))
            run_C += denominator
...
if saveF:
    ...
    for i in np.linspace(0.5,6,100):
        ...
        plt.grid()
        plt.title(f'Velocity Expectation: Maxwell Boltzmann Distribution with $C = 1$, $\\lambda = ${round(i,3)}')
        plt.xlabel('Step')
